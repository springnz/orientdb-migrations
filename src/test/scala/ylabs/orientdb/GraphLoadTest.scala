...
class GraphLoadTest extends WordSpec with ShouldMatchers {
  ...
  def createVertices(count: Int, label: Labels.Value)(properties: => Map[String, AnyRef]): Unit = {
    (1 to count) foreach { i ⇒
      ...
      val propsList = properties.map { case (key, value) => Seq(key, value) }.flatten.toSeq
  ...
  "tinkerpop api" should {
    "create scenario graph" in {
      ...
      val userCount = 10
      time {
        createVertices(userCount, User){ Map("url" -> s"http://us.co.nz/${math.random}") }
        // createVertices(userCount, User)
        // addSessions(userCount * 3)
        // createVertices(20, Listing)
        // createListingViews(userCount * 6)
        // createViewNumberEvents(userCount * 2)
      }
